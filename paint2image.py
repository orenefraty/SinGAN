        ...
        if (opt.paint_start_scale < 1) | (opt.paint_start_scale > (len(Gs)-1)):
            ...
            if opt.quantization_flag:
                ...
                if (os.path.exists(dir2trained_model)):
                    ...
                    opt.mode = 'paint2image'
            out1 = SinGAN_generate(Gs[n:], Zs[n:], reals1, NoiseAmp[n:], opt, [1,0], in_s, n=n, num_samples=1)
            out2 = SinGAN_generate(Gs[n:], Zs[n:], reals2, NoiseAmp[n:], opt, [0,1], in_s, n=n, num_samples=1)
            plt.imsave('%s/img1_start_scale=%d.png' % (dir2save, opt.paint_start_scale), functions.convert_image_np(out1.detach()), vmin=0, vmax=1)
            plt.imsave('%s/img2_start_scale=%d.png' % (dir2save, opt.paint_start_scale), functions.convert_image_np(out2.detach()), vmin=0, vmax=1)
